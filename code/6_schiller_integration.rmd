...
```{r}
schiller_anndata$layers
```

```{r}
head(schiller_anndata$layers["matrix"])
```
...
ggplot() + umap_theme() +
        ggrastr::rasterise(geom_point(data = test2, aes(x = x, y = y, color = cell_type_res4), size = .1), dpi = 600) +
...
plosa_epi$lab <- "plosa"
DefaultAssay(plosa_epi) <- "RAW_COUNTS"
plosa_epi <- DietSeurat(plosa_epi, assays = "RAW_COUNTS")
plosa_epi <- RenameAssays(plosa_epi, RAW_COUNTS = "RNA")
...
merged_data <- merge(DietSeurat(schiller_obj_essential, assays = "RNA"), plosa_epi)
...
data_list <- SplitObject(merged_data, split.by = "lab")
...
```{r}
saveRDS(epi_data_combined_all, "./data/b1/20211216_schiller_plosa_integrated.rds", compress = "bzip2")
```
...
ggsave("./data/b1/figures/prelim_schiller/all_yesintegration.png", a + b + c + d & theme(aspect.ratio = 1), scale = 1.5)
ggsave("./data/b1/figures/prelim_schiller/all_gex_yesintegration.png", fplt & theme(aspect.ratio = 1), scale = 1.5)
...
```{r}
plosa_epi_original <- readRDS("./data/b1/b1_labeled_epithelium.rds")
```
...
transfer_anchors <- FindTransferAnchors(reference = epi_data_list$schiller, query = plosa_epi_original,
...
predictions <- TransferData(anchorset = transfer_anchors, refdata = epi_data_list$schiller$cell_type_recombined,
                            dims = 1:30)
plosa_epi_original <- AddMetaData(plosa_epi_original, metadata = predictions)

```

```{r}
DimPlot(plosa_epi_original, group.by = "predicted.id")
```

```{r}
DimPlot(subset(plosa_epi_original, genotype == "B1"), group.by = "predicted.id")
```
```{r}
DimPlot(subset(plosa_epi_original, genotype == "WT"), group.by = "predicted.id")
```
```{r}
saveRDS(plosa_epi_original, "./data/b1/b1_labeled_epithelium_lab_transfer.rds")
```
